...
public class Pawn extends Figure {
	...
	protected int checkMoveCorrect(Figure[][] board, XY to) {
		...
		boolean isAbleToGoStraightForwardUntouchedTwoMoves = (this.isTouched() == false && stepX == 0 && stepY * direction ==2);
		boolean isAbleToGoStraightForwardOneMove = (stepX == 0 && (stepY * direction == 1));
		boolean isAbleToTakeFigure = (Math.abs(stepX) == 1) ? isAbleToTakeFigure(board, to, direction) : false;
		
		boolean result = isAbleToGoStraightForwardOneMove || isAbleToGoStraightForwardUntouchedTwoMoves || isAbleToTakeFigure;
	...
	protected boolean isNoFigureOnTheWay(Figure[][] board, XY to) {
		...
		if (stepX == 0){
			for (int i = 1; i <= stepY * direction; i++){
				Figure fig = board[this.getXY().getX()][this.getXY().getY() + i * direction];
				if (fig != null)
					return false;
			}
			return true;
			...
			boolean isAbleToTakeFigure = isAbleToTakeFigure(board, to, direction);
	...
	private boolean isAbleToTakeFigure(Figure[][] board, XY to, int direction){
		...
		for (XY xy : removableEnemysXY){
			isAbleToTakeFigure = xy.equals(to) && board[xy.getX()][xy.getY()] != null && board[xy.getX()][xy.getY()].getRank().getOwner() != this.getRank().getOwner();
			if (isAbleToTakeFigure == true)
				break;
		}
